...
from data_combined import GraphDataset, TextGraphDataset, GloVeTokenizer
import utils
from evaluation_combined import eval_link_prediction
...
def link_prediction(dataset, dataset_setting, dim, model, rel_model, loss_fn,
    ...
    _log.info(f'output_directory is: {output_directory}')
    _log.info(f'use_scheduler is: {use_scheduler}')
    _log.info(f'regularizer is: {regularizer}')
    ...
    if model == 'transductive':
        ...
        if model.startswith('bert') or model.startswith('raild'):
        ...
        train_data = TextGraphDataset(triples_file, num_negatives,
                                      ...
                                      node2vec_embeddings=f'dataset_creation/datasets/{dataset_setting}/{dataset}/rel_embeddings.txt',
                                      write_maps_file=True,
    ...
    for epoch in range(1, max_epochs + 1):
        train_loss = 0
        for step, (text_tok_ent, text_mask_ent, text_tok_rel, text_mask_rel, relation_features, rels, neg_idx) in enumerate(train_loader):
            loss = model(text_tok_ent=text_tok_ent, text_mask_ent=text_mask_ent,
                         text_tok_rel=text_tok_rel, text_mask_rel=text_mask_rel,
                         relation_features=relation_features, rels=rels, neg_idx=neg_idx).mean()
