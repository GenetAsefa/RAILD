...
from data_node2vec import GraphDataset, TextGraphDataset, GloVeTokenizer
import utils
from evaluation_node2vec import eval_link_prediction
...
def link_prediction(dataset, dataset_setting, dim, model, rel_model, loss_fn,
    ...
    drop_stopwords = model in {'bert-bow', 'bert-dkrl', 'glove-bow', 'glove-dkrl',
                             'bow-node2vec', 'dkrl_node2vec'}
    ...
    for epoch in range(1, max_epochs + 1):
        train_loss = 0
        for step, (text_tok_ent, text_mask_ent, relation_features, rels, neg_idx) in enumerate(train_loader):
