...
class TextGraphDataset(GraphDataset):
    ...
    def __init__(self, triples_file, neg_samples, max_len, tokenizer,
                 drop_stopwords,node2vec_embeddings, write_maps_file=False, use_cached_ent_text=False,
        ...
        # load relation features from rel-rel-net
        #### while generating features, the embeddings for train, valid, and test relatios should all be saved in one file.

        self.node2vec_rels = torch.zeros((len(rel_ids), 768),
                                     dtype=torch.float)
        #print(self.node2vec_rels)
        self.load_node2vec_embeddings(rel_ids, node2vec_embeddings)
    ...
    def load_node2vec_embeddings(self, rel_ids, node2vec_embeddings):
        loaded = set()
        progress = tqdm(desc='Loading relation embeddings/features', ## loading from file node2vec embeddings
                        total=len(rel_ids), mininterval=5)

        file_path = osp.join(node2vec_embeddings)
        if not osp.exists(file_path):
        ...
        for line in lines:
            values = line.strip().split(' ')
            relation = values[0]
            embedding = torch.FloatTensor([float(i) for i in values[1:]])
            if relation not in rel_ids:
                continue
            if relation in loaded:
                continue

            loaded.add(relation)
            rel_id = rel_ids[relation]


            # Starting slice of row contains embeddings
            self.node2vec_rels[rel_id, :] = embedding
        ...
        if len(loaded) != len(rel_ids):
            raise ValueError(f'Loaded {len(loaded):,} embeddings,'
                             f' but {len(rel_ids):,} were expected.')
    ...
    def get_relation_description(self, rel_ids):
        ...
        #print(text_tok_rel.shape)
        return text_tok_rel, text_mask_rel, text_len_rel

    def get_relation_features(self, rel_ids):
        """Get realtion features for a tensor of relation IDs."""
        node2vec_rels = self.node2vec_rels[rel_ids].float()
        node2vec_rels = torch.squeeze(node2vec_rels, 1)

        return node2vec_rels
    ...
    def collate_fn(self, data_list):
        ...
        relation_features = self.get_relation_features(rels)

        neg_idx = get_negative_sampling_indices(batch_size, self.neg_samples,
                                                repeats=self.num_devices)


        return text_tok_ent, text_mask_ent, text_tok_rel, text_mask_rel, relation_features, rels, neg_idx
